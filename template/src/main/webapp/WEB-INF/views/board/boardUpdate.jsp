...
<body>
  ...
  <form action="${pageContext.request.contextPath }/boardUpdate" method="post">
    <!-- ../boardUpdate해도 됨 -->
    <div>
      글번호 <input name="bno" value="${board.bno }" readonly="readonly">
    </div>
    <div>
      제목 <input name="title" placeholder="제목" value="${board.title }">
    </div>
    <div>
      내용 <input name="content" placeholder="내용" value="${board.content }">
    </div>
    <div>
      작성자 <input name="writer" placeholder="작성자" value="${board.writer }" readonly="readonly">
    </div>
    <div>
      등록일 <input name="regDate" placeholder="등록일" readonly="readonly" value='<fmt:formatDate value="${board.regDate }" pattern="yyyy-MM-dd hh:mm" />'>
    </div>
    <button>수정</button>
  ...
  <form>
    <input name="" id="">작성자 <input name="" id="">내용
